...
contract GovernanceSlashingTest is BaseTest {
    ...
    function test_hyperlaneCanSlashUpToNetworkLimit() public {
        ...
        deal(address(spk), alice, 60_000_000e18);
        deal(address(spk), bob,   40_000_000e18);

        vm.startPrank(alice);
        spk.approve(address(sSpk), 60_000_000e18);
        sSpk.deposit(alice, 60_000_000e18);
        vm.stopPrank();

        vm.startPrank(bob);
        spk.approve(address(sSpk), 40_000_000e18);
        sSpk.deposit(bob, 40_000_000e18);
        vm.stopPrank();
        ...
        uint256 slashIndex = slasher.requestSlash(subnetwork, OPERATOR, 100_000_000e18, captureTimestamp, "");
        ...
        assertEq(sSpk.activeBalanceOf(alice), 60_000_000e18);
        assertEq(sSpk.activeBalanceOf(bob),   40_000_000e18);
        assertEq(sSpk.totalStake(),           100_000_000e18);
        assertEq(sSpk.activeStake(),          100_000_000e18);

        assertEq(slasher.slashableStake(subnetwork, OPERATOR, captureTimestamp, ""), 2_000_000e18);

        assertEq(spk.balanceOf(address(sSpk)), 100_000_000e18);
        assertEq(spk.balanceOf(BURNER_ROUTER), 0);

        vm.prank(HYPERLANE_NETWORK);
        slasher.executeSlash(slashIndex, "");

        assertEq(sSpk.activeBalanceOf(alice), 60_000_000e18 - 1_200_000e18);  // Proportional slash
        assertEq(sSpk.activeBalanceOf(bob),   40_000_000e18 - 800_000e18);  // Proportional slash
        assertEq(sSpk.totalStake(),           98_000_000e18);
        assertEq(sSpk.activeStake(),          98_000_000e18);

        assertEq(slasher.slashableStake(subnetwork, OPERATOR, captureTimestamp, ""), 0);

        assertEq(spk.balanceOf(address(sSpk)), 98_000_000e18);
    ...
    function test_ownerMultisigCanVetoSlash() public {
        ...
        // --- Step 3: Owner multisig vetoes the slash request
