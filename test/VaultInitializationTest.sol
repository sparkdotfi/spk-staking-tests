...
//     bytes32 private constant REQUEST_SLASH_SIG =
//         keccak256("RequestSlash(uint256,bytes32,address,uint256,uint48,uint48)");
//     bytes32 private constant EXECUTE_SLASH_SIG =
//         keccak256("ExecuteSlash(uint256,uint256)");
//     bytes32 private constant VETO_SLASH_SIG =
//         keccak256("VetoSlash(uint256,address)");
//     bytes32 private constant SET_RESOLVER_SIG =
...
//         // scan them for all VetoSlasher event signatures
//         for (uint i = 0; i < allLogs.length; i++) {
//             bytes32 sig = allLogs[i].topics[0];
//             assertTrue(
//                 sig == REQUEST_SLASH_SIG ||
//                 sig == EXECUTE_SLASH_SIG ||
//                 sig == VETO_SLASH_SIG ||
//                 sig == SET_RESOLVER_SIG,
//                 "Unknown VetoSlasher event found!"
//             );
//         }
//     }
// }
...
//         assertEq(allLogs[1].topics[0], OWNERSHIP_TRANSFERRED_SIG);
//     }
// }



//     // NETWORK_DELEGATOR event signatures (additional to existing ones)
...
//     bytes32 private constant CLAIM_SIG =
//         keccak256("Claim(address,address,uint256,uint256)");
//     bytes32 private constant CLAIM_BATCH_SIG =
//         keccak256("ClaimBatch(address,address,uint256[],uint256)");
//     bytes32 private constant ON_SLASH_VAULT_SIG =
//         keccak256("OnSlash(uint256,uint48,uint256)");
//     bytes32 private constant SET_DEPOSIT_WHITELIST_SIG =
//         keccak256("SetDepositWhitelist(bool)");
//     bytes32 private constant SET_DEPOSITOR_WHITELIST_STATUS_SIG =
//         keccak256("SetDepositorWhitelistStatus(address,bool)");
//     bytes32 private constant SET_IS_DEPOSIT_LIMIT_SIG =
//         keccak256("SetIsDepositLimit(bool)");
//     bytes32 private constant SET_DEPOSIT_LIMIT_SIG =
//         keccak256("SetDepositLimit(uint256)");
...
//     bytes32 private constant APPROVAL_SIG =
...
//                 sig == CLAIM_SIG ||
//                 sig == CLAIM_BATCH_SIG ||
//                 sig == ON_SLASH_VAULT_SIG ||
//                 sig == SET_DEPOSIT_WHITELIST_SIG ||
//                 sig == SET_DEPOSITOR_WHITELIST_STATUS_SIG ||
//                 sig == SET_IS_DEPOSIT_LIMIT_SIG ||
//                 sig == SET_DEPOSIT_LIMIT_SIG ||
